 ...
 */
@WebServlet(name = "NuevoCajero", urlPatterns = {"/NuevoCajero"})
public class NuevoCajero extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (nombre.trim().equals("")) {
            ...
            Cajero nuevoCajero = new Cajero(codigoUsuario, nombre, DPI, direccion, sexo, password, 2, turno, horaEntrada, horaSalida, true);

            //INSTANCIA DE LA CLASE ENCARGADA DE REGISTRAR NUEVOS CLIENTES
            AccesoANuevoCajero registrar = new AccesoANuevoCajero();
            ...
            if (registrar.insertarNuevoCajero(nuevoCajero)) {
                registrar.insertarCreacionHistorialCajero(nuevoCajero);
                System.out.println("SI SE INGRESO TODO CORRECTAMENTE");
                request.setAttribute("Exitoso", true);
                request.setAttribute("informeRegistro", nuevoCajero);
            } else {
                request.setAttribute("Exitoso", false);
            }

        }
        RequestDispatcher despachar = request.getRequestDispatcher("NuevoCajero_G.jsp");
